...
function OpenCloakroomMenu()
  ...
  if PlayerData.job.grade_name == 'officer' then
    ...
    table.insert(elements, {label = '👮‍ Polisuniform - T-Shirt', value = 'insats_wear'})
  end
...
function OpenArmoryMenu(station)
  ...
  if Config.EnableArmoryManagement then
    ...
    local elements = {
      ...
      {label = '⚖️ Ta ut beslagtagna föremål',  value = 'get_stock'},
...
function OpenVehicleSpawnerMenu(station, partNum)
  ...
  if Config.EnableSocietyOwnedVehicles then
    ...
    local elements = {}
      ...
      table.insert(elements, { label = '🚔 Radiobil: Volkswagen T6', value = 'policeold1'})
    ...
    table.insert(elements, { label = '🚔 Radiobil: Volvo V70', value = 'fbi'})
    table.insert(elements, { label = '🚔 Radiobil: Volvo V90', value = 'police2'})
    table.insert(elements, { label = '🚔 Polisbuss: Volkswagen Transporter 6', value = 'riot'})
    table.insert(elements, { label = '🚔 Civilbil: Volvo XC70', value = 'sheriff'})  
        ...
        ESX.UI.Menu.Open(
          'default', GetCurrentResourceName(), 'citizen_interaction',
          {
            ...
            elements = {
			  ...
			  {label = ('👨‍⚕️ Hjärt & Lungräddning'),   value = 'revive'},
...
AddEventHandler('esx_ambulancejob:drag', function(ambulance)
  IsDragged = not IsDragged
  AmbulancePed = tonumber(ambulance)
end)
...
Citizen.CreateThread(function()
  while true do
    Wait(0)
    if IsDragged then
      local ped = GetPlayerPed(GetPlayerFromServerId(AmbulancePed))
...
Citizen.CreateThread(function()
    ...
    Citizen.Wait(1)
         ...
         if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 454.7, -1017.4, 28.4, true) <= 2.5 then
            Draw3DText(454.7, -1017.4, 28.4, 'Tryck [~g~E~w~] för att ta ut ett ~r~tjänstefordon~w~.')
            end
        end
    end
end)
...
Citizen.CreateThread(function()
    ...
    Citizen.Wait(1)
         ...
         if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 462.3, -1019.6, 28.1, true) <= 2.5 then
            Draw3DText(462.3, -1019.6, 28.1, 'Tryck [~g~E~w~] för att lagra ditt ~r~tjänstefordon~w~.')
            end
        end
    end
end)
...
Citizen.CreateThread(function()
    ...
    Citizen.Wait(1)
         ...
         if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 462.6, -1014.4, 28.1, true) <= 2.5 then
            Draw3DText(462.6, -1014.4, 28.1, 'Tryck [~g~E~w~] för att lagra ditt ~r~tjänstefordon~w~.')
            end
        end
    end
end)
...
Citizen.CreateThread(function()
    ...
    Citizen.Wait(1)
         ...
         if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 460.9, -982.4, 30.6, true) <= 2.5 then
            Draw3DText(460.9, -982.4, 30.6, 'Tryck [~g~E~w~] för att ta ut ditt ~r~tjänstevapen~w~.')
            end
        end
    end
end)
...
Citizen.CreateThread(function()
    ...
    Citizen.Wait(1)
         ...
         if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 450.0, -992.8, 30.6, true) <= 2.5 then
            Draw3DText(450.0, -992.8, 30.6, 'Tryck [~g~E~w~] för att byta ~r~outfit~w~.')
            end
        end
    end
end)
...
Citizen.CreateThread(function()
    ...
    Citizen.Wait(1)
         ...
         if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 448.4, -973.0, 30.6, true) <= 2.5 then
            Draw3DText(448.4, -973.0, 30.6, 'Tryck [~g~E~w~] för hantera ~r~chefs-menyn~w~.')
            end
        end
    end
end)

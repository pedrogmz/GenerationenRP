    ...
    function(result)
      if result[1] ~= nil then
        ...
        if playerName ~= nil then
          local name = getIdentity(playerName.source)
          cb(name.firstname .. ' ' .. name.lastname, true)
        else
          TriggerClientEvent('esx:showNotification', source, 'Han är ej i staden')
      	end
      else
        cb('unknown', false)
      end
...
ESX.RegisterServerCallback('esx_policejob:getOtherPlayerData', function(source, cb, target)
  ...
  if Config.EnableESXIdentity then
    ...
    local height        = user['height']--heightFeet .. "\' " .. heightInches .. "\""
        ...
        if foundIdentifier ~= nil then
          ...
          MySQL.Async.fetchAll(
            ...
            function(result)
              ...
              local LastDigits = result[1].dateofbirth .. '-' ..result[1].lastdigits
              local efterlyst = result[1].wanted

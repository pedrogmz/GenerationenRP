  ...
  function Start(ped)
	  ...
	  while checking do
		  ...
		  if distance < 2.0 then
			  DrawText3D(x,y,z, 'Tryck [~g~E~s~] för att hantera individ', 0.4)
  ...
  function Notification(x,y,z)
	  ...
	  while (timestamp + 4500) > GetGameTimer() do
		  Citizen.Wait(0)
		  DrawText3D(x, y, z, 'Skadan bör ha uppkommit här.', 0.4)
		  checking = false
	  end
  end
  ...
  function OpenDeathMenu(player)
	  ...
	  table.insert(elements, {label = 'Undersök dödsorsaken', value = 'deathcause'})
	  table.insert(elements, {label = 'Undersök vart skadan uppstod', value = 'damage'})
	  ...
	  function(data, menu)
		  ...
		  if ac == 'damage' then
			  ...
			  if success then
				  ...
				  Notify('Vart skadan uppstod är okänt.')
			  end
		  end

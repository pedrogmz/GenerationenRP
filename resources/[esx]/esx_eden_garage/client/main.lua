		...
		function(data, menu)
			if(data.current.value.state)then
				...
				if c > 0 then
					...
					sendNotification(_U('vehicle_is_impounded'), 'error', '1500')
				end
			else
				sendNotification(_U('vehicle_is_impounded'), 'error', '1500')
			end
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			if(data.current.value == 'no') then
				sendNotification(_U('visit_mechanic'), 'error', '1500')
			end
...
function ranger(vehicle,vehicleProps)
	...
	sendNotification(_U('vehicle_in_garage'), 'error', '1500')
end
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		ESX.TriggerServerCallback('eden_garage:stockv',function(valid)
			...
			if (valid) then
				...
				sendNotification(_U('cannot_store_vehicle'), 'error', '1500')
			end
		end,vehicleProps)
	else
		sendNotification(_('no_vehicle_to_enter'), 'error', '1500')
	end
		...
		function(data, menu)
			...
			ESX.TriggerServerCallback('eden_garage:checkMoney', function(hasEnoughMoney)
				if hasEnoughMoney then
					...
					sendNotification(_U('not_enough_money'), 'error', '1500')						
				end
			end)
		end,
		function(data, menu)
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(1)
         if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 126.663, 6608.389, 31.8, true) <= 2.5 then
            Draw3DText(126.663, 6608.389, 31.8, '[~g~E~w~] för att lagra ditt ~r~fordon~w~')
        end
    end
end)
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(1)
         if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 128.732, 6622.979, 31.8, true) <= 2.5 then
            Draw3DText(128.732, 6622.979, 31.8, '[~g~E~w~] för att ta ut ett ~r~fordon~w~')
        end
    end
end)
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(1)
         if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 1878.984, 3753.407, 32.9, true) <= 2.5 then
            Draw3DText(1878.984, 3753.407, 32.9, '[~g~E~w~] för att ta ut ett ~r~fordon~w~')
        end
    end
end)
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(1)
         if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 1887.087, 3767.702, 32.9, true) <= 2.5 then
            Draw3DText(1887.087, 3767.702, 32.9, '[~g~E~w~] för att lagra ditt ~r~fordon~w~')
        end
    end
end)
...
--- Centrum - Ta ut fordon

Citizen.CreateThread(function()
	while true do
		Citizen.Wait(1)
         if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 229.671, -804.911, 30.8, true) <= 2.5 then
            Draw3DText(229.671, -804.911, 30.8, '[~g~E~w~] för att ta ut ett ~r~fordon~w~')
        end
    end
end)

--- Centrum - Lagra fordon

Citizen.CreateThread(function()
	while true do
		Citizen.Wait(1)
         if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 217.847, -783.061, 30.8, true) <= 2.5 then
            Draw3DText(217.847, -783.061, 30.8, '[~g~E~w~] för att lagra ditt ~r~fordon~w~')
        end
    end
end)
...
--- Flygplatsen - Ta ut fordon

Citizen.CreateThread(function()
	while true do
		Citizen.Wait(1)
         if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), -675.402, -2390.109, 13.8, true) <= 2.5 then
            Draw3DText(-675.402, -2390.109, 13.8, '[~g~E~w~] för att ta ut ett ~r~fordon~w~')
        end
    end
end)

--- Flygplatsen - Lagra fordon

Citizen.CreateThread(function()
	while true do
		Citizen.Wait(1)
         if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), -668.807, -2378.282, 13.8, true) <= 2.5 then
            Draw3DText(-668.807, -2378.282, 13.8, '[~g~E~w~] för att lagra ditt ~r~fordon~w~')
        end
    end
end)

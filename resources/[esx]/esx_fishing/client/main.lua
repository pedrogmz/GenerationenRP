...
AddEventHandler('esx_fishing:playFishingAnimation', function()
	...
	CurrentActionMsg  = 'Hold E to cancel fishing'
...
AddEventHandler('esx_fishing:hasEnteredMarker', function(zone)
	...
	if zone == 'SellFish' then
		CurrentActionMsg  = 'Tryck E för att sälja din fisk'
		CurrentAction = ACTION_SELLING_FISH
	end

	if zone == 'FishingLeaderboard' then
		CurrentActionMsg  = 'COMING SOON'
...
Citizen.CreateThread(function()
	while true do
		...
		while FishHooked == FISH_HOOKED and CurrentAction == ACTION_FISHING do
			DrawScreenText('Tryck ~g~SPACE ~w~för att fånga fisken!', {255, 255, 255, 255, 1}, { 0.448, 0.3725 }, { 0.75, 0.75 }, true)
		...
		while CaughtFish ~= nil do
			...
			DrawScreenText("Du fångade en " .. bigOrSmall .. " " .. CaughtFish.sex .. " " .. CaughtFish.name .. " weighing ~g~" .. CaughtFish.weight .. " grams~w~!", {255, 255, 255, 255, 1}, { 0.448, 0.3725 }, { 0.75, 0.75 }, true)
		...
		while DisplayLeaderboard do
			local titleMessage = "Vem är det som leder?"
			local pikeMessage = "Pike - Ingen fångad idag!"
			local salmonMessage = "Salmon - Ingen fångad idag!"
			local bassMessage = "Bass - Ingen fångad idag!"

			if CurrentPikeLeaderboad[1] ~= nil then
				pikeMessage = "Pike - ~g~" .. CurrentPikeLeaderboad[1].owner_name .. "~w~ - " .. CurrentPikeLeaderboad[1].weight .. " Gram"
			end
			if CurrentSalmonLeaderboad[1] ~= nil then
				salmonMessage = "Salmon - ~g~" .. CurrentSalmonLeaderboad[1].owner_name .. "~w~ - " .. CurrentSalmonLeaderboad[1].weight .. " Gram"
			end
			if CurrentBassLeaderboad[1] ~= nil then
				bassMessage = "Bass - ~g~" .. CurrentBassLeaderboad[1].owner_name .. "~w~ - " .. CurrentBassLeaderboad[1].weight .. " Gram"
			end

...
function JailLogin()
	...
	ESX.ShowNotification("Senaste gången du somnade så var du i fängelset, Därför är du här.")
...
function UnJail()
	...
	ESX.ShowNotification("Du är nu friad, Vi har kvar dig i registret så ta det lugnt.")
end
		...
		while jailTime > 0 do
			...
			for posId, v in pairs(Packages) do
				...
				if DistanceCheck <= 10.0 then
					...
					local PackageText = "Paketera"

					if not v["state"] then
						PackageText = "Redan Paketerad"
					end
		...
		while true do
			...
			for p, v in pairs(Config.Teleports) do
				...
				if DistanceCheck <= 7.5 then
					...
					ESX.Game.Utils.DrawText3D(v, "[E] Öppna dörren", 0.4)
...
function PackPackage(packageId)
	...
	while Packaging do
		...
		if PackPercent >= 100 then
			...
			ESX.Game.Utils.DrawText3D(Package, "Paketerar... " .. math.ceil(tonumber(PackPercent)) .. "%", 0.4)
		end
...
function OpenJailMenu()
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'jail_prison_menu',
		{
			...
			align    = 'top-left',
			elements = {
				{ label = "Fängsla närmsta person", value = "jail_closest_player" },
				{ label = "Fria närmsta person", value = "unjail_player" }
			}
		}, 
	function(data, menu)
			...
			ESX.UI.Menu.Open(
          	...
          	function(data2, menu2)
            	...
            	if jailTime == nil then
              		ESX.ShowNotification("Tiden ska vara i minuter.")
            	else
              		menu2.close()
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
				...
				if #playerArray == 0 then
					ESX.ShowNotification("Din cell är tom!")
					return
				end

				for i = 1, #playerArray, 1 do
					table.insert(elements, {label = "Fånge: " .. playerArray[i].name .. " | Fängelsetid: " .. playerArray[i].jailTime .. " minuter", value = playerArray[i].identifier })
				end

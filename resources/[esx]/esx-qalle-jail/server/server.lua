	...
	if xPlayer["job"]["name"] == "police" then
		...
		if GetPlayerName(jailPlayer) ~= nil then
			...
			if jailTime ~= nil then
				...
				TriggerClientEvent("esx:showNotification", src, GetPlayerName(jailPlayer) .. " fängslad i " .. jailTime .. " minuter!")
				
				if args[3] ~= nil then
					GetRPName(jailPlayer, function(Firstname, Lastname)
						TriggerClientEvent('chat:addMessage', -1, { args = { "TINGSRÄTTEN",  Firstname .. " " .. Lastname .. " Är nu fängslad, Anledning: " .. args[3] }, color = { 249, 166, 0 } })
					end)
				end
			else
				TriggerClientEvent("esx:showNotification", src, "Tiden är ogiltlig.")
			end
		else
			TriggerClientEvent("esx:showNotification", src, "ID:t du angav är inte inne.")
		end
	else
		TriggerClientEvent("esx:showNotification", src, "Du är inte en polis.")
	end
end)
...
AddEventHandler("esx-qalle-jail:jailPlayer", function(targetSrc, jailTime, jailReason)
	...
	GetRPName(targetSrc, function(Firstname, Lastname)
		TriggerClientEvent('chat:addMessage', -1, { args = { "TINGSRÄTTEN",  Firstname .. " " .. Lastname .. " Är nu fängslad, Anledning: " .. jailReason }, color = { 249, 166, 0 } })
	end)

	TriggerClientEvent("esx:showNotification", src, GetPlayerName(targetSrc) .. " Fängslad i " .. jailTime .. " minuter!")
end)
...
AddEventHandler("esx-qalle-jail:prisonWorkReward", function()
	...
	TriggerClientEvent("esx:showNotification", src, "Bra jobbat, Här får du lite pengar.")
end)

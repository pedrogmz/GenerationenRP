		...
		if not InBlacklistedVehicle then
			...
			if(nearPump and IsCloseToLastVehicle) then
				...
				if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Kliv ut ur bilen för att tanka.")
				elseif IsFueling then
					local position = GetEntityCoords(vehicle)

					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Tryck ~g~G ~w~för att ställa tillbaka pumpen i maskinen. SEK~r~" .. price .. " ~w~+  Moms")
				...
				elseif fuel > 95.0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Ditt fordon har redan tillräckligt med bränsle.")
				elseif cash <= 0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Försöker du lura oss, Du har inte tillräckligt med pengar på dig.")
				else
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Tryck ~g~G ~w~för att tanka ditt fordon SEK~r~16.31/~w~Litern + Moms.")
			...
			elseif NearVehicleWithJerryCan and not nearPump then
				...
				if IsFuelingWithJerryCan then
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Tryck ~g~G ~w~för att sluta tanka. Just nu på: " .. fuel .. "% - Bensindunk: " .. jerrycan)
					...
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Tryck ~g~G ~w~för att tanka ditt fordon med en bensindunk.")
...
Citizen.CreateThread(function()
	while true do
		if IsFueling then
			...
			price = price + fuelthis * 16.31 * 1.1
...
function DisplayHud()
	if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
		...
		DrawAdvancedText(0.188 - x, 0.9850 - y, 0.128, 0.0028, 0.5, fuel, 255, 255, 255, 255, 6, 1)

		DrawAdvancedText(0.148 - x, 0.9850 - y, 0.128, 0.0028, 0.5, "~y~BRÄNSLE:", 255, 255, 255, 255, 4, 1)
	end
end

...
local gym = {
    {x = -1195.109,y = -1577.452,z = 4.6115}
}
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(gym) do
            DrawMarker(21, gym[k].x, gym[k].y, gym[k].z, 0, 0, 0, 0, 0, 0, 1.501, 1.501, 1.001, 0, 153, 255, 255, 0, 0, 0, 0)
        end
    end
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(rentbike) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Tryck ~INPUT_CONTEXT~ för att hyra en ~b~cykel.')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(gym) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Tryck ~INPUT_CONTEXT~ för att öppna ~b~gym~w~ menyn.')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(arms) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Tryck ~INPUT_CONTEXT~ för att träna ~g~armar.')
				
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
					
						TriggerServerEvent('esx_gym:checkChip')
						ESX.ShowNotification("Förbereder ~g~övning~w~...")
						Citizen.Wait(1000)					
						...
						if membership == true then
							...
							ESX.ShowNotification("Du behöver vila ~r~60 sekunder ~w~innan du kan påbörja en ny övning.")
							
							--TriggerServerEvent('esx_gym:trainArms') ## COMING SOON...
							
							training = true
						elseif membership == false then
							ESX.ShowNotification("Du behöver ett medlemsskap för att ~r~träna.")
						end
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(chins) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Tryck ~INPUT_CONTEXT~ för att ~g~pull-ups')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(pushup) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Tryck ~INPUT_CONTEXT~ för att göra ~g~armhävningar.')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(yoga) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Tryck ~INPUT_CONTEXT~ för att träna ~g~yoga')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(situps) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Tryck ~INPUT_CONTEXT~ för att träna ~g~armhävningar')
...
function CheckTraining()
	...
	if resting == false then
		ESX.ShowNotification("Du har nu vilat klart...")
	end
end
...
function OpenGymMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_menu',
        {
            title    = 'Gym',
            elements = {
				{label = '🛒 Butik', value = 'shop'},
				{label = '📋 Öppet-tider', value = 'hours'},
				{label = '📝 Medlemsskap', value = 'ship'},
            }
        },
        function(data, menu)
...
function OpenGymShopMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_shop_menu',
        {
            title    = 'Gym - Shop',
            elements = {
				{label = 'Protein Shake (12SEK)', value = 'protein_shake'},
				{label = 'Vatten (10SEK)', value = 'water'},
				{label = 'Sportlunch (12SEK)', value = 'sportlunch'},
				{label = 'Powerade (15SEK)', value = 'powerade'},
            }
        },
        function(data, menu)
...
function OpenGymShipMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_ship_menu',
        {
            ...
            elements = {
				{label = 'Membership (500SEK)', value = 'membership'},
            }
        },
        function(data, menu)

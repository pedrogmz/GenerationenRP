...
function kockped()
        ...
        kock = CreatePed(5, modelHash, -1195.177, -893.632, 13.095, -45.0, false, false)
        kockSpawned = true
        end
        Wait(200)
        kockSpawned = false
...
function servped()
    ...
    serv = CreatePed(5, modelHash, -1195.304, -897.100, 13.095, -45.0, false, false)
    servSpawned = true
    end
    Wait(200)
    servSpawned = false
...
Citizen.CreateThread(function()
	while true do Citizen.Wait(0)
		if (GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), -1193.668, -892.577, 13.095, true) <= 2.1 ) then
			if IsControlPressed(0, Keys['E']) then

				local elements = {
                    {label = 'BBQ Bacon King (SEK 47)', value = 'happymeal', item = 'hamburgare', price = 47},
				}
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'didrik_restaurant', {
				    ...
				    if data.current.value == 'happymeal' then
                      ESX.UI.Menu.CloseAll()
                      kockped()
                      sendNotification('Tar emot beställning..', 'error', 3500)
                      ...
                      TaskGoStraightToCoord(kock, -1195.304, -897.100, 13.095, 1.0, 100000, 0.0, 2.0)  
                      Wait(1500)
                      deletekock()
                          blockinput = true
                          local cam = CreateCam("DEFAULT_SCRIPTED_CAMERA", 1)
                          SetCamCoord(cam, -1193.401, -888.021, 15.0)
                              SetCamRot(cam, 0.0, 0.0, 160.0, 2)
                          RenderScriptCams(1, 0, 0, 0, 0)
                              Citizen.Wait(500)
                      TimeLeft = Config.TimeToMat
                      repeat                                	
                      TriggerEvent("mt:missiontext", 'Din mat tillagas, Vänligen vänta. Tid kvar: ~r~' .. TimeLeft .. ' ~w~sekunder', 1000)
                      TimeLeft = TimeLeft - 1
                      Citizen.Wait(1000)
                      until(TimeLeft == 0)	  
                      servped()   
                      FreezeEntityPosition(serv, false)
                      TaskGoStraightToCoord(serv, -1195.287, -893.599, 13.095, 1.0, 100000, 0.0, 2.0)      
                      Citizen.Wait(3500)   
                      ...
                      sendNotification('Du äter upp maten..', 'error', 5500)
                      Wait(9000)
                      blockinput = false
...
--- Mat meny
  ...
  while true do
	Citizen.Wait(1)
		...
		if (GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), -1193.668, -892.577, 13.095, true) <= 1000 ) then
			DrawMarker(27, -1193.668, -892.577, 13.095, 0, 0, 0, 0, 0, 0, 2.001, 2.0001, 0.5001, 204, 204, 204, 200, 0, 0, 0, 0)
		end
  
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), -1193.668, -892.577, 13.980, true) <= 6.5 then
			Draw3DText(-1193.668, -892.577, 13.980, '[~g~E~w~] för att öppna ~r~menyn~w~')
		end
	end
  end)
...
--- Blip
Citizen.CreateThread(function()
    local blip = AddBlipForCoord(-1184.499, -884.837, 45.841)
    ...
    AddTextComponentString('Burger King')

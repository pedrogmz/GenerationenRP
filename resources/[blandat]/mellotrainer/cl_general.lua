...
function teleportToWaypoint()
	...
	if(not IsWaypointActive())then
		drawNotification("Du har inte satt ut någon marker.")
		return
	end
...
Citizen.CreateThread( function()
	while true do
		...
		if ( IsControlJustReleased( 0, 108 ) or IsDisabledControlJustReleased( 0, 108 ) ) and GetLastInputMethod( 0 ) and not IsPauseMenuActive() and not blockinput and ((settings["adminOnlyTrainer"] == true and adminStatus == true) or settings["adminOnlyTrainer"] == false) then -- f1
		...
		if ( IsControlJustReleased( 0, 111 ) or IsDisabledControlJustReleased( 0, 111 ) ) and GetLastInputMethod( 0 ) and not blockinput and ((settings["adminOnlyTrainer"] == true and adminStatus == true) or settings["adminOnlyTrainer"] == false) then -- f3
			teleportToWaypoint()
		end

		if ( IsControlJustReleased( 0, 112 ) or IsDisabledControlJustReleased( 0, 112 ) ) and GetLastInputMethod( 0 ) and ((settings["adminOnlyNoclip"] == true and adminStatus == true) or settings["adminOnlyNoclip"] == false) then
			toggleNoClipMode()
		end

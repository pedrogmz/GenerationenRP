...
admins = {
    'steam:1100001169feb5e',
    'steam:110000114a63805',
...
RegisterCommand('freezetime', function(source, args)
    if source ~= 0 then
        if isAllowedToChange(source) then
            ...
            if freezeTime then
                TriggerClientEvent('vSync:notify', source, 'Tiden är nu ~b~fryst~s~.')
            else
                TriggerClientEvent('vSync:notify', source, 'Tiden är inte ~y~längre fryst~s~.')
            end
        else
            TriggerClientEvent('chatMessage', source, '', {255,255,255}, '^8UNITED-ROLEPLAY: ^1Du har inte tillåtelse att använda detta command.')
        end
    else
        freezeTime = not freezeTime
        if freezeTime then
